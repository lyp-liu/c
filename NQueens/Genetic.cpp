...
Genetic::Genetic(int numOfQueens, int initialGroupNum) {
    if(numOfQueens<30)
        initialGroupNum=numOfQueens*20;
    else initialGroupNum=200;
    m_adaptive.resize(initialGroupNum, 0);
    m_AdaptValue.resize(initialGroupNum, 0);
    m_Num = numOfQueens;
    m_IsSuccess = true;
    SetPopulation();//设定初始种群
}
...
double Genetic::CalcuAdaptive(vector<int>& state) {//计算适应值
    int conflict = 0;//记录冲突数
...
void Genetic::Choose() {
    ...
    for (int i = 1; i < m_AdaptValue.size(); i++)//用前一步的累积适应值加当前单步的适应值更新总累积适应值
    ...
    for (int i = 0; i < m_population.size(); i++) {
        if (rand() % 2) {
            ...
            if (first % 2 == 0) {
                ...
                for (int j = crossPoint; j < m_Num; j++)
                    swap(m_population[row1][j], m_population[i][j]);  //值交换
            }
            else row1 = i;
        }
    }
...
vector<vector<string>> Genetic::GeneticCalculation() {
    ...
    while (m_IsSuccess) {//当m_IsSuccess为假时说明已经找到最终状态，停止循环
        // 自然选择
        Choose();
        // 杂交
        GeneticCrossover();
